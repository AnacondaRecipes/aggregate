...
REPLACEMENTS = {
  ...
  'chromium-clang-format': 'buildtools/linux64/clang-format',
...
def DoMain(argv):
  ...
  for lib, path in REPLACEMENTS.items():
    ...
    if args.undo and os.path.exists(os.path.join(source_tree_root, path + '.orig')):
    ...
    elif os.path.exists(os.path.join(source_tree_root, path)):
